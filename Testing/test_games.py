...
def test_rolling_uses_shift1():
    ...
    assert abs(expected6 - got6) < 1e-6, (
...
def test_rest_days_computed_from_schedule():
    ...
    df["game_date"] = pd.to_datetime(["2024-01-01", "2024-01-03", "2024-01-04",
...
def test_opponent_rolling_correctly_joined():
    ...
    for game_id in df["game_id"].unique():
        ...
        if pd.notna(team1_row["opp_rolling_win_pct_10"]) and \
           pd.notna(team2_row["team_rolling_win_pct_10"]):
            assert abs(
                team1_row["opp_rolling_win_pct_10"] - team2_row["team_rolling_win_pct_10"]
            ) < 1e-9, "Opponent rolling features not correctly mirrored"
