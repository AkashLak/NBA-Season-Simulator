...
def test_std_dev_pie_single_player_returns_zero():
    single = pd.DataFrame({
        "team_id":     [1], "season_year": [2023], "player_id": [10],
        "games_played":[60], "minutes_pg": [35.0], "pie": [0.15], "age": [28],
    })
...
def test_first_season_per_team_has_nan_lag():
    ...
    assert first_rows["prev_wins"].isna().all(), \
...
def _make_ml_df():
    return pd.DataFrame({
        "team_id": [1], "season_year": [2023],
        "wins": [47], "wins_normalized": [47.0],
        "off_rating": [115.0], "def_rating": [112.0], "net_rating": [3.0],
        "pts_pg": [115.0], "ts_pct": [0.57], "ast_to_ratio": [2.1],
        "pace": [100.0], "oreb_pct": [0.25], "dreb_pct": [0.75],
        "tm_tov_pct": [0.13], "team_avg_pie": [0.11], "playoff_team": [True],
        "team_avg_age": [27.0], "roster_turnover_pct": [0.30],
        "avg_games_played": [65.0], "star_age_flag": [False],
        "std_dev_pie": [0.025], "top_3_minutes_share": [0.72],
    })
...
def test_build_feature_row_midseason_does_not_raise():
    ml_df = _make_ml_df()
    partial = {c: 1.0 for c in [
        "net_rating", "off_rating", "def_rating", "pts_pg",
        "ts_pct", "ast_to_ratio", "pace", "oreb_pct",
        "dreb_pct", "tm_tov_pct", "team_avg_pie", "avg_games_played",
    ]}
