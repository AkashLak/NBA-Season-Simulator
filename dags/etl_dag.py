...
def _ingest_season(**context):
    ...
    print(
        f"Season ingest complete: {len(raw['team_stats'])} team rows, "
...
def _ingest_games(**context):
    ...
    from etl.ingest_games import SEASONS, ingest_games
    from etl.load import save_to_parquet
...
def _load_all(**context):
    ...
    from etl.transform import normalize_player_stats, normalize_team_stats
    ...
    upsert_to_postgres(
        team_df, "nba_team_season_stats", engine, conflict_cols=["team_id", "season"]
    )
    upsert_to_postgres(
        player_df,
    ...
    upsert_to_postgres(
        ml_df, "ml_training_features", engine, conflict_cols=["team_id", "season_year"]
    )
...
def _evaluate_and_gate(**context):
    ...
    with open(_LAST_TRAINED_PATH, "w") as f:
        json.dump(
            {"season_year": int(max_season), "trained_at": datetime.now().isoformat()},
            f,
        )
