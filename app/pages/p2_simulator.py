...
from app.shared import (
    load_ml_features, load_player_stats, load_models, load_game_report,
    get_engine, team_selector, season_selector, wins_color, no_data_warning, LAKERS_ID,
)
...
if roster_df.empty:
    st.warning(f"No player data for this team/season. Run ETL to populate.")
    st.stop()

# ── Diminishing returns helper ─────────────────────────────────────────────────
def adjusted_pie(player_B: pd.Series, player_A: pd.Series) -> float:
    role_increase = max(0.0, float(player_A.get("minutes_pg", 20))
                            - float(player_B.get("minutes_pg", 20)))
...
def recompute_team_features(roster: pd.DataFrame) -> dict:
    qualified = roster[roster.get("games_played", pd.Series(dtype=float)) >= 10].copy() \
        if "games_played" in roster.columns else roster.copy()
    if qualified.empty:
    ...
    team_avg_pie = float((qualified["pie"] * qualified["min_share"]).sum()) \
        if "pie" in qualified.columns else 0.10

    total_team_min = roster["minutes_pg"].sum() if "minutes_pg" in roster.columns else 0
    top3_min = roster.nlargest(3, "minutes_pg")["minutes_pg"].sum() \
        if "minutes_pg" in roster.columns else 0
    top_3_minutes_share = float(top3_min / total_team_min) if total_team_min > 0 else 0.7

    new_min = roster[~roster["player_id"].isin(original_ids)]["minutes_pg"].sum() \
        if "player_id" in roster.columns and "minutes_pg" in roster.columns else 0
...
display_cols = [c for c in ["player_name", "age", "games_played", "minutes_pg", "pie"]
                if c in roster_df.columns]
st.dataframe(roster_df[display_cols].sort_values("minutes_pg", ascending=False)
             if "minutes_pg" in roster_df.columns else roster_df[display_cols],
...
all_players = player_df[
    player_df["season_year"] == player_df["season_year"].max()
].copy().sort_values("minutes_pg", ascending=False) if "minutes_pg" in player_df.columns \
    else player_df.copy()

current_roster_ids = set(roster_df["player_id"].tolist()) if "player_id" in roster_df.columns else set()
candidates = all_players[~all_players["player_id"].isin(current_roster_ids)] \
    if "player_id" in all_players.columns else all_players
...
for idx, row in roster_df.iterrows():
    ...
    with st.expander(f"Swap: {pname}  (PIE: {pie_val}, Min: {row.get('minutes_pg', '?'):.0f})"):
        ...
        swap_choice = st.selectbox(
            ...
            format_func=lambda pid: "-- No Change --" if pid == "-- No Change --"
                else candidates[candidates["player_id"] == pid]["player_name"].iloc[0]
                if "player_name" in candidates.columns else str(pid),
        ...
        with st.spinner("Simulating league…"):
            ...
            try:
                result = simulate_team(
                    team_id, predict_season, game_model, get_engine(),
                    team_quality_override={"prev_net_rating": adj_net_rating,
                                           "prev_off_rating": float(prior_row.iloc[0].get("off_rating", 110)) + delta_pie * net_rating_scale * 0.5,
                                           "prev_def_rating": float(prior_row.iloc[0].get("def_rating", 110)) - delta_pie * net_rating_scale * 0.5},
                )
