...
from app.shared import load_game_report, load_learning_curve, load_season_report
    ...
    with st.expander("Cross-validation results (all candidates)", expanded=True):
        ...
        if cv_results:
            ...
            if not winner_row.empty:
                st.caption(
                    f"Winner: **{game_report.get('winner', '').upper()}** "
                    "-- lowest CV log loss with best holdout AUC."
                )
        else:
